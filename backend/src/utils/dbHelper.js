...
// Convierte placeholders de MySQL (?) a PostgreSQL ($1, $2, ...)
function adaptQuery(query, params = []) {
  ...
  // Reemplazar funciones específicas de MySQL
...
// Ejecuta query adaptándola al motor correcto
async function executeQuery(pool, query, params = []) {
  ...
  if (isPostgres) {
    ...
    return [result.rows]; // Devuelve en formato MySQL [rows]
  } else {
    return await pool.query(adaptedQuery, adaptedParams);
  }
}

// Convierte valores booleanos
